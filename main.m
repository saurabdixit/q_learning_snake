...
while 1
    ...
    Q_matrix_fruit(ind_previous,3+action) = Q_matrix_fruit(ind_previous,3+action) + learning_rate * (reward_for_moving + discount_factor * max(Q_matrix_fruit(ind_current,4:end)) - Q_matrix_fruit(ind_previous,3+action));
    Q_matrix_fruit(ind_previous,3+action)
    pause;
    if sum(sum(world==1.5)) == 1
        fruit_eaten = 1;
        world(fruit_r,fruit_c) = 1;

        ind = sub2ind(size(world), snake_location(1,1), snake_location(1,2));
        Q_matrix_fruit(ind,3+action) = Q_matrix_fruit(ind,3+action) + learning_rate * reward_for_fruit;

        snake_location = append_snake(snake_location,previous_snake_element_location);
        [fruit_r,fruit_c] = spawn_fruit(world);
        Global_Q_matrix = update_global_q_fruit(Q_matrix_fruit, Global_Q_matrix, grid_size, fruit_r, fruit_c);
    end
